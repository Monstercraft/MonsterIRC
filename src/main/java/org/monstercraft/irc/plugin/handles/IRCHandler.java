...
public class IRCHandler {
	 ...
	 */
	public boolean connect(IRCServer server) {
		this.server = server;
		if (connection != null) {
	...
	public void join(final IRCChannel channel) throws IOException {
		...
		IRCJoinEvent jevent = new IRCJoinEvent(channel, getServer().getNick());
		plugin.getServer().getPluginManager().callEvent(jevent);
		PluginJoinEvent pje = new PluginJoinEvent(channel, getServer()
				.getNick());
	...
	public void part(final IRCChannel channel) {
		...
		IRCPartEvent levent = new IRCPartEvent(channel, getServer().getNick());
		plugin.getServer().getPluginManager().callEvent(levent);
		PluginPartEvent ppe = new PluginPartEvent(channel, getServer()
				.getNick());
